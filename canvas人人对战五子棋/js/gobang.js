...
function drwTable() {
	...
	cxt.beginPath();
	cxt.fillStyle = "black";
	cxt.arc(175,175,5,0,Math.PI*2);
	cxt.fill(); // 进行绘制
	cxt.beginPath();
	cxt.arc(575,175,5,0,Math.PI*2);
	cxt.fill(); // 进行绘制
	cxt.beginPath();
	cxt.arc(175,575,5,0,Math.PI*2);
	cxt.fill(); // 进行绘制
	cxt.beginPath();
	cxt.arc(575,575,5,0,Math.PI*2);
	cxt.fill(); // 进行绘制
}
...
function winW() {
	cxt.beginPath();
	cxt.font = 'bolder 120px 微软雅黑 ';
	cxt.fillStyle = 'black';
	cxt.textAlign = 'center';
	cxt.textBaseline = 'top';
	cxt.fillText('白棋赢!', 375, 270);
	cxt.fill();
}
//黑棋子赢
function winB() {
...
btn1.onclick = function() {
	gameStart();
	c.style.background = 'rgba(152,103,53,0.8)';
}
